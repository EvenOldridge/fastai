...
# nn.Embedding(padding_idx=0) <- Causes the embedding to output all 0's,
# ?which means the weights are 0 in the case of relu

# This means I should be able to pad the input sequence defining the padding_idx as \n
# and the NN will ignore sequences that are longer than that length.

# But will that mean if I tie the output that the sequences will try to target this 0'd vector?
# I think so...  :-(

# I think the only way around this is a custom loss function that explicitly ignores padded variables
# or I need to use torch.nn.utils.rnn.pack_padded_sequence() which I'm not sure is compatible with Fast.ai

# Custom loss function route makes the most sense I think.
# with it I can simply set loss to 0 for padded sequences which will prevent gradient updates.
# it's not as efficient per se but it avoids more complex coding updates.

# I think I either need to overload the autograd backward function which Jeremy is already doing
# and even better I'm pretty sure I can just call super.backward() if it's not a padding but that's
# returning the gradient, which I suppose if it's 0 then the update will be no change.
# http://pytorch.org/docs/master/notes/extending.html

# One concern here is that the error will be artificially deflated.


# In order to calculate the vector directly rather than through cross_entropy it is necessary to
# detach the embedding output.
# https://discuss.pytorch.org/t/using-word-embedding-with-loss-function/9902/6
